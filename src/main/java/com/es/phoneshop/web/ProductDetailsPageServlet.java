...
public class ProductDetailsPageServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (lastViewedProducts.size() > 3) {
            lastViewedProducts.subList(3, lastViewedProducts.size()).clear();
        }
        session.setAttribute(PRODUCTS, lastViewedProducts);
...
//            return;
//        }
        cartService.add(cart, productId, quantity); // Здесь убрал try catch, но на фронте перестало выводится сообщение об ошибке, стало просто перекидывать на error page, и сообщение ошибки выводить в консоль
